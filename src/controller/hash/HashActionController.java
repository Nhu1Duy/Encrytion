...
public class HashActionController {
	...
	public void bind() {
		ctx.view.hashPanel.getConfigPanel().getHashTextBtn().addActionListener(e -> handleHashText());
	}
	...
	private void handleHashText() {
		...
		try {
			...
			ctx.view.setStatus("Hash OK – " + ctx.view.hashPanel.getCurrentAlgo() + " (" + result.length() * 4 + " bit)");
		} catch (Exception ex) {
	...
	private HashFactory.HashAlgorithm mapAlgo(String displayName) {
		switch (displayName) {
		case "MD2":
			return HashFactory.HashAlgorithm.MD2;
		case "SHA1":
			return HashFactory.HashAlgorithm.SHA1;
		case "SHA-224":
			return HashFactory.HashAlgorithm.SHA224;
		case "SHA-256":
			return HashFactory.HashAlgorithm.SHA256;
		case "SHA-384":
			return HashFactory.HashAlgorithm.SHA384;
		case "SHA-512":
			return HashFactory.HashAlgorithm.SHA512;
		case "RIPEMD160":
			return HashFactory.HashAlgorithm.RIPEMD160;
		case "Blake2b-256":
			return HashFactory.HashAlgorithm.BLAKE2B;
		default:
			return HashFactory.HashAlgorithm.MD5;
		}
	}
}